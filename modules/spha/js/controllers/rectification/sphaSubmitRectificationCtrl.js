/**
 * @ngdoc function
 * @name sphaSubmitRectificationCtrl
 * @description controller for search rectification
 * # sphaSubmitRectificationCtrl Controller of the sphaApp
 */
(function () {
    'use strict';
            ...
            ['PropertiesServiceSpha', '$rootScope', '$stateParams', '$state', '$scope', '$window', '$cookies', '$q', '$translate', 'httpServices',
                ...
                function (PropertiesServiceSpha, 
                    ...
                    function createPreviewFile() {
                    	...
                    	if(pdfRequest) {
                    		sphaRectificationServices.getRectificationPdfPreview(pdfRequest, function (data, error) {
                    ...
                    function getRectification() {
                    	...
                    	if(pdfRequest) {
                    		sphaRectificationServices.getRectificationById(pdfRequest.idRectification, function (data, error) {
                        		if(data) {
                    ...
                    function updateRectification(isSubmitEvent) {
            			...
            			sphaRectificationServices.updateRectification(vm.rectificationDTO, function (data, error) {
                    		if (error && error.message) {
                                vm.message = error.message;
                                vm.alertClass = error.alertClass
                            } else {
                    ...
                    function deletePreviouslyCreatedFiles(isSubmitEvent) {
                    	vm.fileToDelete.forEach(idFile => {
            				sphaRectificationServices.deleteRectificationFile(idFile, function (data, error) {
                        		if (error && error.message) {
                                    vm.message = error.message;
                    ...
                    function addNewFiles(filesToDelete, isSubmitEvent) {
                    	...
                    	var rectificationFileDTO = {
            					rectificationId: vm.rectificationDTO.id,
                    			category: 'DOC_SUBMIT',
                    			type: 'ATTACHMENTS'
                    	};
            			
            			vm.files.forEach(file => {
            				rectificationFileDTO.description = file.name;
                			uploadServicesSpha.uploadOneOptionalMultipartFileWithData(file,
                					rectificationFileDTO, 'meta', apiRectificationFileUrl, function (data, success, error) {
                				if (error) {
            			...
            			sphaRectificationServices.submitRectification(pdfRequest, function (idFile,  errorSubmit) {
                			...
                			if(!idFile){
                				...
                				var signRequest = {
                					    ...
                					    fileParams: [{
                					        "entityId": pdfRequest.idRectification, // id rettifica
