/**
 * @ngdoc function
 * @name sphaPaybackAifaAcceptanceCtrl
 * @description controller for search payback data
 * # sphaPaybackAifaAcceptanceCtrl Controller of the sphaApp
 */
(function () {
    'use strict';
    angular.module('sphaApp')
        .controller('sphaPaybackAifaAcceptanceCtrl', sphaPaybackAifaAcceptanceCtrl);
        
        sphaPaybackAifaAcceptanceCtrl.$inject = [ '$scope', 'NgTableParams', '$cookies', '$sce', 'blockUI',
        '$document', '$compile', '$q', '$state', '$rootScope', '$window', 'shareDataServices', 'sphaPaybackServices', 'loadingSpinnerService',
    ...
    function sphaPaybackAifaAcceptanceCtrl ( $scope, NgTableParams, $cookies, $sce, blockUI,
        $document, $compile, $q, $state, $rootScope, $window, shareDataServices, sphaPaybackServices, loadingSpinnerService,
        sphaUtilsServices, sphaCompanyServices, cityServicesSpha, httpServices, SweetAlert, $translate, PropertiesServiceSpha, sphaProcedureInstanceServices ) {
        ...
        vm.procedureInstanceId = $state.params['procedureInstanceId'];
        vm.isLoading = false;
        ...
        var sortingKeyCookie = 'sortKey';
        var sortingValueCookie = 'sortValue';
       ...
       function updateCookie(cookieName, cookieValue) {
        ...
        function getData(obj) {
            var deferred = $q.defer();
            sphaPaybackServices.getModuloAccettazionePaged(
                obj, vm.procedureId, vm.procedureInstanceId, function (data, success, error) {
        ...
        vm.deleteAcceptanceFile = function(id) {
            SweetAlert.swal({
                ...
                if (isConfirm) {
                    sphaPaybackServices.deleteAcceptance(id, function (data, error) {
                        if(data) {
                            vm.amountTable.reload();
                        } else {
                            vm.message = error.message;
        ...
        vm.deleteAllAcceptanceFile = function() {
            SweetAlert.swal({
                title: $translate.instant('CONFIRM_DELETE_GROUP'),
                text: null,
                type: 'warning',
                ...
                if (isConfirm) {
                    ...
                    sphaPaybackServices.deleteAllAcceptance(vm.selectedEntities, function (data, error) {
                        if(data) {
                            vm.amountTable.reload();
                            vm.selectedEntities = [];
        ...
        vm.canExport = function() {
            return !vm.procedureInstanceId;
        }

        vm.exportReport = function () {
            var obj = {
                filters: {
                    procedureId: {equals: vm.procedureId},
                },
                unpaged: true
            };
        ...
        $scope.selectAll = function () {
            ...
            if ($scope.allItemsSelected){
                for (var i = 0; i < vm.data.length; i++) {
                    vm.selectedEntities.push(vm.data[i].id);
                    vm.selectedEntities = Array.from(new Set(vm.selectedEntities))
                }
            }else{
                vm.selectedEntities = [];
            }
        ...
        $scope.rowSelected = function (entity) {	  
            return vm.selectedEntities.includes(entity.id);
        }      	 		            
        
        $scope.selectEntity = function (entity) {	        	 		            
            if (!$scope.rowSelected(entity)){
                vm.selectedEntities.push(entity.id);
            }else{
                var index = vm.selectedEntities.indexOf(entity.id);
                if (index > -1) {
                    vm.selectedEntities.splice(index, 1);
                }		            	
            }
            
            $scope.allItemsSelected = !vm.data.some(row => !$scope.rowSelected(row));
            
        };

        function exportData(obj) {
            var deferred = $q.defer();
            httpServices.post(sphaPaybackServices.getExportReportUrl('PB5_AIFA_AIC_9_ACC', {type: 'PB5'}), obj, function (data, success, error) {
                if (success) {
                    deferred.resolve({
                        data: data
                    });
