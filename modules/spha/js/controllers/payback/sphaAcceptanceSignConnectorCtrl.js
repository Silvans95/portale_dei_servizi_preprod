...
/**
 * @ngdoc function
 * @name sphaAcceptanceSignConnectorCtrl
 * @description controller for sign Connector - ACCEPTANCE Entities
 * # sphaAcceptanceSignConnectorCtrl
 */
(function () { 
 'use strict';
	angular.module('sphaApp')
	.controller('sphaAcceptanceSignConnectorCtrl',
		['$location','PropertiesServiceSpha', '$state', '$window', 'httpServices', 'sphaPaybackServices', 'shareDataServices', 'SweetAlert', '$translate', 'sphaUtilsServices',
			function ( $location, PropertiesServiceSpha, $state, $window, httpServices, sphaRectificationServices, shareDataServices, SweetAlert, $translate, sphaUtilsServices ) {
				var vm = this;
				vm.message = "";
				vm.alertClass = "";
				vm.data = {};
				...
				signConnectorUrl = urlProcedure + "api/modulo-accettaziones/" + sessionId + "/sign-fet-post"
				...
				if(signConnectorUrl && signConnectorUrl !== '') {
                    ...
                    httpServices.post(signConnectorUrl, null, function (data, success, error) {
                        ...
                        var requestProtocolFile = {
                         		idProcedureInstance: pdfRequest.procedureInstanceId,
                         		idDocument: versionId,
                         		idModuloAccettazione: pdfRequest.id
                         }
                        requestProtocolFile.anagraphicType = param;
                        if(success && versionId) {
                        	sphaRectificationServices.protocollAcceptanceFile(requestProtocolFile, function (protocol_success, protocol_error) {
                                if (JSON.stringify(protocol_success) != "{}" && protocol_success != null){
	                            	...
	                            	redirect('spha.pb5ProcedureInstanceView');
                                } else {
                                	var errorProtocol = "";
									if(protocol_success) {
                                        ...
                                        if (protocol_error.message === "ERROR_FILE_ALREADY_SIGNED") {
                                            originPage = 'spha.pb5ProcedureInstanceView';
                                        }
									}
                                	SweetAlert.swal({
                        	...
                        	shareDataServices.set(true, 'RECTIFICATION_PROTOCOL_PAGE');
                        	redirect('spha.submitAcceptance');                      
                        }
