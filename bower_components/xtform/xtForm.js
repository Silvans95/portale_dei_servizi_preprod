    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            function init(){
            	...
            	var ngModel = null;
            	scope.$watch( function() {
            		return attrs['for'] || attrs.xtValidationInline;
            	}, function (inputId) {
            		if( inputId ){
...
xtForm.directive('xtValidationSummary', ["$templateCache", '$document', function ($templateCache, $document) {
    'use strict';
    ...
    return {
        ...
        link: function (scope, element, attrs, ctrls) {
                ...
                angular.forEach(form, function (ngModel, ngModelKey) {
                    if (ngModelKey[0] !== '$') {
                        ...
                        angular.forEach(errors, function (value) {
                            scope.errors.push({
                                key: value.key,
                                label: ngModel.$label ? ngModel.$label : getLabelByName( ngModel.$name ),
            ...
            function getLabelByName( name ){
               var labelEl = $document[0].querySelectorAll('label[for="' + name + '"]');
               return labelEl.length > 0 ? labelEl[0].innerText : '';
            }
